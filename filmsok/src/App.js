  ...
  async function info(sok) {
    ...
    if (data.Search) {
      const filmPromises = data.Search.map (async (result) =>{
        const filmRespons = await fetch(`https://www.omdbapi.com/?apikey=b56c90e7&i=${result.imdbID}&plot=full`);    //https://www.pluralsight.com/guides/process-an-api-response-in-react
